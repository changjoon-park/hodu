...
//
// Buffer Layout:
// - buffer(0): input tensor
// - buffer(1): condition (bool array)
// - buffer(2): output tensor
// - buffer(3): metadata
// - buffer(4): atomic counter for output position

#define COMPRESS_OP(TYPENAME, FN_NAME)                                                             \
    kernel void hodu_metal_##FN_NAME(                                                              \
        const device TYPENAME *input [[buffer(0)]], const device bool *condition [[buffer(1)]],    \
        device TYPENAME *output [[buffer(2)]], constant size_t *metadata [[buffer(3)]],            \
        device atomic_uint *counter [[buffer(4)]], uint id [[thread_position_in_grid]]) {          \
                                                                                                   \
        ...
        const size_t axis_value = metadata[2 + 2 * num_dims + 3];                                  \
                                                                                                   \
        if (axis_flag == 0) {                                                                      \
            /* Flatten mode: id is condition index */                                              \
            if (id >= condition_size)                                                              \
                return;                                                                            \
            if (!condition[id])                                                                    \
                return;                                                                            \
                                                                                                   \
            ...
            /* Axis mode: id is condition index (axis slice index) */                              \
            if (id >= condition_size)                                                              \
                return;                                                                            \
            if (!condition[id])                                                                    \
                return;                                                                            \
            ...
            for (size_t i = 0; i < condition_size; i++) {                                          \
                if (condition[i])                                                                  \
                    num_selected++;                                                                \
                                                                                                   ...
                                                                                                   \
            /* Copy entire slice: iterate over outer and inner dimensions */                       \
            for (size_t outer = 0; outer < outer_size; outer++) {                                  \
                for (size_t inner = 0; inner < slice_size; inner++) {                              \
                    ...
                    size_t out_flat =                                                              \
                        outer * num_selected * slice_size + out_slice_idx * slice_size + inner;    \
                                                                                                   \
...
COMPRESS_OP(bfloat, compress_bf16)
COMPRESS_OP(half, compress_f16)
